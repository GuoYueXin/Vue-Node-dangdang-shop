            ...
            <ul class="cart-item-list">
              <li v-for="(item,index) in cartList" :key="index">
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        ...
                        <span class="select-ipt">{{item.productedNum}}</span>
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">￥{{parseFloat(item.price.slice(1))*item.productedNum}}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a href="javascript:;" class="item-edit-btn" @click="mdShow=true;bookId=item.bookId">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="selectAll()">
                  <span class="checkbox-btn item-check-btn" :class="{'check':selectCkeckAll}">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis':selectCount==0}" @click="toAddress()">提交</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
    ...
    export default{
        ...
        computed:{
          selectCkeckAll(){
            return this.selectCount == this.cartList.length
          },
          selectCount(){
            var i = 0;
          ...
          totalPrice(){
            ...
            this.cartList.forEach((ele)=>{
              if(ele.checked == "1"){
                money += (parseFloat(ele.price.slice(1))*parseInt(ele.productedNum));
              }
            })
        ...
        methods:{
          //初始化方法
          init(){
            var self = this;    //注意this指向，在axios中this默认指向本身，而不是上级，可以在外面定义this传递进去，或者使用箭头函数解决
            axios.get('/users/cartList').then((Response)=>{
              let res = Response.data;
              if(res.status == "0"){
                this.cartList = res.result;
                // this.cartList.forEach(function(ele){
                //   self.totalPrice += (parseFloat(ele.price.slice(1))*ele.productedNum);
                // })
              }else{
                console.log(111)
              }
          ...
          editCart(flag,item){
            ...
            axios.post('/users/cartEdit',{
              bookId:item.bookId,
              productedNum:item.productedNum,
          ...
          selectAll(){
            var flag = !this.selectCkeckAll;
              this.cartList.forEach((ele)=>{
                 ele.checked = flag?'1':'0';
              })
