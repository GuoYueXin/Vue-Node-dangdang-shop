            ...
            <ul class="cart-item-list">
              <li v-for="(item, index) in cartList" :key="index" v-if="item.checked == 1">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">￥{{(parseFloat(item.price.slice(1))*parseInt(item.productedNum)).toFixed(2)}}</div>
                </div>
              </li>
            </ul>
          </div>
          ...
          axios.get('/users/getCartList').then((Response)=>{
            ...
            if(res.status == "0"){
              ...
              this.cartList.forEach((ele)=>{
                if(ele.checked == "1"){
                  this.itemSubTotal += ((parseFloat(ele.price.slice(1))*parseInt(ele.productedNum)));
                  // console.log((parseFloat(ele.price.slice(1))*parseInt(ele.productedNum)).toFixed(2))
                }
              })
              this.orderTotal += this.itemSubTotal + parseFloat(this.shipping) - parseFloat(this.discount);
