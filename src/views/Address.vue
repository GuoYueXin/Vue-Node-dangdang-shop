        ...
        <div class="container">
          <div class="checkout-addr">
            <div class="page-title-normal">
              <h2 class="page-title-h2"><span>查看订单</span></h2>
            </div>
            <!-- process step -->
            ...
            <div class="addr-list-wrap">
                ...
                <ul>
                  <li v-for="(item,index) in addressListFilter" :key="index" :class="{'check':checkIndex == index}" @click="checkIndex=index;selectAddrId = item.addressId;">
                    ...
                    <div class="addr-opration addr-del">
                      <a href="javascript:;" class="addr-del-btn" @click="addressId = item.addressId;mdShow = true">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default md-show" v-show="!item.isDefault">
                  ...
                  <li class="addr-new" @click="addMdShow = true">
              ...
              <div class="shipping-addr-more" v-show="addressList.length > 3">
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="changeLimit()" :class="{'open':limit > 3}">
                  more
                  <i class="i-up-down">
            ...
            <div class="shipping-method-wrap">
              <div class="shipping-method">
                <ul>
                  <li class="check">
                    <div class="name">普通快递</div>
                    <div class="price">￥{{shipping}}</div>
                    <div class="shipping-tips">
                      <p>快递发出后，将在1-7个工作日内到达您的手中</p>
                    </div>
                  </li>
                </ul>
              </div>
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :class="{'btn--dis':addressList.length == 0}" :to="{path:'orderConfrim',query:{'addressId':selectAddrId}}">下一步</router-link>
            </div>
          </div>
        </div>
      </div>
      <Model :mdShow="mdShow" v-on:close="closeModel">
        <p slot="message">确认删除此地址吗？</p>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="delAddress(addressId)">删除</a>
          <a href="javascript:;" class="btn btn--m" @click="mdShow = false">关闭</a>
        </div>
      </Model>
      <Model :mdShow="addrWarnMd" v-on:close="closeModel">
        <p slot="message">您还没有添加配送地址哟</p>
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="addrWarnMd=false;addMdShow=true">添加</a>
          <a href="javascript:;" class="btn btn--m" @click="addrWarnMd = false">关闭</a>
        </div>
      </Model>
      <div class="md-modal modal-msg md-modal-transition" :class="{'md-show':addMdShow}">
        <div class="md-modal-inner">
          <div class="md-top">
            <div class="md-title">添加地址</div>
            <button class="md-close" @click="addMdShow=false;errorTip=false">关闭</button>
          </div>
          <div class="md-content">
             <div class="error-wrap">
                <span class="error error-show" v-show="errorTip">你又调皮了，快递小哥去外星找你吗</span>
              </div>
              <ul>
                <li class="regi_form_input">
                  姓名：<input type="text" class="regi_login_input" v-model="name">
                </li>
                <li class="regi_form_input noMargin" >
                  手机号： <input type="text" class="regi_login_input" v-model="tel">
                </li>
                <li class="regi_form_input noMargin">
                  街道：<input type="text" class="regi_login_input" v-model="streetName">
                </li>
              </ul>
            </div>
            <div class="login-wrap">
              <a href="javascript:;" class="btn-add" @click="addrAdd">添加</a>
            </div>
          </div>
        </div>

        <div class="md-overlay" v-if="addMdShow" @click="addMdShow=false;errorTip=false"></div>
  ...
  export default{
      ...
      methods:{
        init(){
            ...
            if(res.status == "0"){
              if(res.result.length == 0){
                this.addrWarnMd = true;
                return;
              }
              this.addressList = res.result;
              this.addressList.forEach((ele)=>{
                if(ele.isDefault){
                  this.selectAddrId = ele.addressId;
                }
              })
              if(this.addressList.length == 1 && !this.addressList[0].isDefault){
        ...
        setDefault(addressId){
          if(addressId){
            axios.post('/users/changeDefAddr',{
              addressId:addressId
            }).then((Response)=>{
              let res = Response.data;
              if(res.status == "0"){
        ...
        closeModel(){
          this.mdShow = false;
          this.addrWarnMd = false;
        },
        addrAdd(){
          if(this.name == "" || this.tel == "" || this.streetName == ""){
            this.errorTip = true;
            return;
          }
          axios.post('users/addAddr',{
            name:this.name,
            tel:this.tel,
